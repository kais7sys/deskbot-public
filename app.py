...
def logout():
    ...
    st.session_state.chat_session = None # Clear AI memory on logout
    st.rerun()
...
def main_app():
    ...
    # 🔧 RENAMED TOOL to force AI to learn new logic
    def add_task_to_scheduler(task_title: str, duration_minutes: int, due_date: str):
        """
        Adds a task to the database.
        task_title: The name of the task. If user says 'at 7pm', put '7pm' here.
        duration_minutes: Duration in minutes (e.g. 60). Defaults to 60 if unknown.
        due_date: YYYY-MM-DD.
        """
        try:
            ...
            return f"✅ Created task: '{task_title}' ({duration_minutes}m) due {due_date}"
    ...
    def ask_agent(user_msg, context, image_data=None):
        try:
            ...
            prompt_parts = [
                ...
                RULES:
                1. If user says a TIME (e.g. '7pm'), include it in the 'task_title'. 
                2. Do NOT put '7pm' in 'duration_minutes'. Duration must be an integer (default 60).
                """,
                f"CONTEXT:\n{context}",
                f"USER: {user_msg}"
            ]
    ...
    # --- SIDEBAR (THE FEATURES YOU MISSED) ---
    with st.sidebar:
        st.caption(f"User: {email}")
        
        # 1. NOTEBOOK SELECTOR
        st.header("1. Select Notebook")
        ...
        if not tasks_df.empty:
            ...
            choice = st.selectbox("Choose Context", options_list)
        ...
        # 2. FILES & UPLOADS
        if selected_task_id:
            st.header(f"2. Files for '{selected_task_title}'")
            ...
            if task_docs:
                for d in task_docs:
                    ...
                    if c2.button("🗑️", key=f"d{d['id']}"): delete_document(d['id']); st.rerun()
            else:
            ...
            if up_file:
                ...
                # CASE B: IMAGE (Show Preview)
                else:
                    st.image(Image.open(up_file), caption="Image Ready for Chat", width=200)
        else:
            st.info("👈 Select a Notebook above to upload files.")
    ...
    tab1, tab2, tab3 = st.tabs(["💬 Agent Chat", "📝 Grid", "📅 Calendar"])

    with tab1: # CHAT TAB (Default)
        # Load History
